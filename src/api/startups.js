...
export async function getStartups(filters = {}) {
  ...
  if (filters.teamSize) {
    ...
    results = results.filter((s) => s.teamSize >= min && s.teamSize <= max);
  }
  if (filters.query) {
    const q = filters.query.toLowerCase();
    results = results.filter((s) =>
      `${s.name} ${s.description} ${s.industry} ${s.tagline}`.toLowerCase().includes(q)
    );
  }
  return results;
}
...
export async function getAiSummary(startupId) {
  await new Promise((r) => setTimeout(r, 800));
  const startup = mockStartups.find((s) => s.id === Number(startupId));
  return startup?.aiSummary || "Summary unavailable.";
}
