...
export async function getInvestorById(id) {
  ...
  return mockInvestors.find((inv) => inv.id === Number(id)) || null;
}
...
export async function getInvestorPortfolio(investorId) {
  ...
  const inv = mockInvestors.find((i) => i.id === Number(investorId));
  if (!inv) return [];
  return mockStartups.filter((s) => inv.portfolioIds.includes(s.id));
}
